...
dna_object <- readDNAStringSet(
  fs::path_package("extdata",
                   "arabidopsis_chloroplast.fa",
...
counts <-
  lapply(bases, function(x) {
    sum(grepl(x, raw_seq_string))
  })
probs <-
  unlist(lapply(counts, function(base_count) {
...
random_lengths <-
  unlist(
    lapply(
      1:10,
      get_longest_orf_in_random_genome,
      length = seq_length,
      probs = probs,
      bases = bases
    )
